  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <p className="text-slate-500 dark:text-slate-400 text-sm mt-1 mb-4">
        {isPro ? 'All premium features are unlocked.' : <>Unlock <span className="font-display font-extrabold tracking-wide uppercase text-orange-500">Ace</span> and let the engine write your training week.</>}
      </p>
      ...
      {isPro ? (
        <div className="border border-slate-200 dark:border-slate-700 rounded-xl p-4 bg-white dark:bg-slate-900">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">You have access to all features including <span className="font-display font-extrabold tracking-wide uppercase text-orange-500">Ace</span>.</p>
        </div>
      ) : (
        <div className="grid md:grid-cols-2 gap-5">
          ...
          <div className="border-2 border-orange-500 rounded-xl p-4 bg-white dark:bg-slate-900">
            ...
            <ul className="space-y-2.5 text-sm text-slate-600 dark:text-slate-300">
              {PRO_FEATURES.map(f => (
                <li key={f} className="flex items-start gap-2">
                  <Check size={15} strokeWidth={2.5} className="text-orange-500 mt-0.5 shrink-0" />{f}
                </li>
              ))}
            </ul>
